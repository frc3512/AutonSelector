...
int main() {
    ...
    autonSelector.AddMode("Auto 1", std::bind(&Robot::initFunc1, &robot),
                          std::bind(&Robot::periodicFunc1, &robot));
    autonSelector.AddMode("Auto 2", std::bind(&Robot::initFunc2, &robot),
                          std::bind(&Robot::periodicFunc2, &robot));
    autonSelector.AddMode("Auto 3", std::bind(&Robot::initFunc3, &robot),
